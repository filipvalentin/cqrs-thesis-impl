...
namespace Lunatic.Application.Features.Teams.Events {
	public class TeamMemberAddedDomainEventHandler(
		...
		ILogger<TeamMemberAddedDomainEventHandler> logger,
		IEventQueueService queueService,
		IUserReadSideRepository userReadRepository) : INotificationHandler<TeamMemberAddedDomainEvent> {
		...
		private readonly ILogger<TeamMemberAddedDomainEventHandler> logger = logger;
		private readonly IEventQueueService queueService = queueService;

		public async Task Handle(TeamMemberAddedDomainEvent notification, CancellationToken cancellationToken) {
			...
			team.MemberIds.AddIfNotExists(notification.MemberId);

			var user = userResult.Value;
			user.TeamIds.AddIfNotExists(notification.Id);
